    ...
    {runId && <div className="objective-layout"><aside className="panel objective-browser">
      ...
      {detail && draft && <>
        ...
        <form className="panel objective-context" onSubmit={save}><h2>Reviewer decision</h2><p>Revision {draft.revision} · {dirty ? 'Unsaved changes' : 'No unsaved changes'}</p><fieldset disabled={busy}><div className="record-grid">
          ...
          <details className="objective-attach"><summary>Attach supporting evidence or records</summary><div className="objective-filters"><label>Resource type<select value={kind} onChange={event => setKind(event.target.value)}>{['SCREENSHOT', 'DISCOVERY', 'ACCOUNT', 'ASSET', 'CHANGE', 'POLICY_EXCEPTION'].map(value => <option key={value}>{value}</option>)}</select></label><label>Search resources<input value={resourceQuery} onChange={event => setResourceQuery(event.target.value)}/></label></div>
