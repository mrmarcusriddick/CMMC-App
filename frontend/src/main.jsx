...
const reviewDefaults = { retentionDays: '', reviewOwner: '', reviewFrequency: '', requiredEventCategories: [], status: 'NOT_STARTED', notes: '' };
const exceptionDefaults = { objectiveIdentifier: '', title: '', rationale: '', compensatingControls: '', owner: '', status: 'DRAFT', expiresAt: '', reviewedBy: '' };
...
function App() {
  ...
  const [page, setPage] = useState(() => window.location.hash.startsWith('#objectives') ? 'objectives' : window.location.hash.startsWith('#summary') ? 'summary' : window.location.hash.startsWith('#poam') ? 'poam' : 'overview');
  ...
  const [exceptionForm, setExceptionForm] = useState(exceptionDefaults);
  ...
  const [auditReview, setAuditReview] = useState(reviewDefaults);
  ...
  async function runAssessment(event) {
    ...
    setRunResult(data); setAuditReview(data.auditReview || reviewDefaults); setMessage('Read-only discovery completed and evidence was recorded locally.'); await loadData();
  }

  async function saveAuditReview(event) {
    event.preventDefault(); setError('');
    const payload = { ...auditReview, retention_days: auditReview.retentionDays === '' ? null : Number(auditReview.retentionDays), review_owner: auditReview.reviewOwner || null, review_frequency: auditReview.reviewFrequency || null, required_event_categories: auditReview.requiredEventCategories, notes: auditReview.notes || null };
    const response = await fetch(`/api/assessments/${runResult.runId}/audit-review`, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(payload) });
    const data = await response.json();
    if (!response.ok) return setError(data.detail || 'Audit review could not be saved.');
    setAuditReview(data); setMessage('Audit evidence review saved locally.');
  }


  async function createException(event) {
    event.preventDefault(); setError(''); setMessage('');
    const payload = { objective_identifier: exceptionForm.objectiveIdentifier || null, title: exceptionForm.title, rationale: exceptionForm.rationale, compensating_controls: exceptionForm.compensatingControls || null, owner: exceptionForm.owner || null, status: exceptionForm.status, expires_at: exceptionForm.expiresAt ? new Date(`${exceptionForm.expiresAt}T12:00:00`).toISOString() : null, reviewed_by: exceptionForm.reviewedBy || null };
    const response = await fetch('/api/exceptions', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(payload) });
    const data = await response.json();
    if (!response.ok) return setError(data.detail || 'Policy exception could not be saved.');
    setExceptionForm(exceptionDefaults); setMessage('Policy exception saved locally. It does not alter the technical assessment.'); await loadData();
  }
  ...
  const filteredExceptions = useMemo(() => exceptions.filter(item => `${item.objectiveIdentifier || ''} ${item.title} ${item.owner || ''} ${item.status}`.toLowerCase().includes(query.toLowerCase())), [exceptions, query]);
  ...
  return <div className="shell">
    ...
    <main className="workspace"><header className="topbar"><div><span className="eyebrow">CMMC LEVEL 2 · GCC HIGH</span><h1>{pageTitle}</h1></div><div className="toolbar"><label className="search">⌕<input value={query} onChange={event => setQuery(event.target.value)} placeholder={page === 'evidence' ? 'Search evidence' : 'Search records'}/></label><button className="export" onClick={() => goToPage('evidence')}>⇩ Evidence exports</button><button className="export" onClick={signOut}>Sign out</button></div></header>
      ...
      {page === 'overview' && <>
        ...
        {runResult && <section className="panel audit-panel"><div className="panel-head"><div><h2>Audit evidence review</h2><p>Assessor-entered context. It never sets compliance automatically.</p></div></div><form className="review-grid" onSubmit={saveAuditReview}><label>Retention period (days)<input type="number" min="0" value={auditReview.retentionDays ?? ''} onChange={event => setAuditReview({...auditReview, retentionDays: event.target.value})}/></label><label>Review owner<input value={auditReview.reviewOwner ?? ''} onChange={event => setAuditReview({...auditReview, reviewOwner: event.target.value})}/></label><label>Review frequency<input placeholder="e.g., monthly" value={auditReview.reviewFrequency ?? ''} onChange={event => setAuditReview({...auditReview, reviewFrequency: event.target.value})}/></label><label>Required event categories<input value={(auditReview.requiredEventCategories || []).join(', ')} onChange={event => setAuditReview({...auditReview, requiredEventCategories: event.target.value.split(',').map(item => item.trim()).filter(Boolean)})}/></label><label>Review outcome<select value={auditReview.status} onChange={event => setAuditReview({...auditReview, status: event.target.value})}>{['NOT_STARTED','IN_REVIEW','SUPPORTED','GAP','NOT_APPLICABLE'].map(status => <option key={status}>{status}</option>)}</select></label><label className="wide">Assessor notes<textarea value={auditReview.notes ?? ''} onChange={event => setAuditReview({...auditReview, notes: event.target.value})}/></label><button>Save audit review</button></form></section>}
      </>}
      ...
      {page === 'exceptions' && <><section className="panel record-form-panel"><div className="panel-head"><div><h2>Record policy exception</h2><p>Time-bound governance record kept separate from technical discovery evidence.</p></div><Status value="DRAFT"/></div><form className="record-grid" onSubmit={createException}><label>Objective identifier<input value={exceptionForm.objectiveIdentifier} onChange={event => setExceptionForm({...exceptionForm, objectiveIdentifier: event.target.value})} placeholder="e.g., IA.L2-3.5.3[a]"/></label><label>Title<input required minLength="3" value={exceptionForm.title} onChange={event => setExceptionForm({...exceptionForm, title: event.target.value})} placeholder="Short exception name"/></label><label>Owner<input value={exceptionForm.owner} onChange={event => setExceptionForm({...exceptionForm, owner: event.target.value})} placeholder="Accountable approver"/></label><label>Status<select value={exceptionForm.status} onChange={event => setExceptionForm({...exceptionForm, status: event.target.value})}>{['DRAFT','SUBMITTED','APPROVED','REJECTED','EXPIRED','CLOSED'].map(value => <option key={value}>{value}</option>)}</select></label><label>Expiration<input type="date" value={exceptionForm.expiresAt} onChange={event => setExceptionForm({...exceptionForm, expiresAt: event.target.value})}/></label><label>Reviewed by<input value={exceptionForm.reviewedBy} onChange={event => setExceptionForm({...exceptionForm, reviewedBy: event.target.value})} placeholder="Reviewer, when applicable"/></label><label className="wide">Business rationale<textarea required minLength="3" value={exceptionForm.rationale} onChange={event => setExceptionForm({...exceptionForm, rationale: event.target.value})} placeholder="Why the exception is needed and its scope"/></label><label className="wide">Compensating controls<textarea value={exceptionForm.compensatingControls} onChange={event => setExceptionForm({...exceptionForm, compensatingControls: event.target.value})} placeholder="Controls that reduce risk during the exception period"/></label><button>Create policy exception</button></form></section><section className="panel table-panel"><div className="panel-head"><div><h2>Policy exception register</h2><p>{filteredExceptions.length} organization-managed records.</p></div></div>{filteredExceptions.length ? <table><thead><tr><th>Objective</th><th>Title</th><th>Owner</th><th>Status</th><th>Expires</th></tr></thead><tbody>{filteredExceptions.map(item => <tr key={item.id}><td><b>{item.objectiveIdentifier || '—'}</b></td><td><b>{item.title}</b><small>{item.reviewedBy ? `Reviewed by ${item.reviewedBy}` : 'Awaiting review'}</small></td><td>{item.owner || '—'}</td><td><Status value={item.status}/></td><td>{fmt(item.expiresAt)}</td></tr>)}</tbody></table> : <Empty title="No policy exceptions" body="Use this register for approved, time-bound deviations and their compensating controls."/>}</section></>}
      {['audit','inventory'].includes(page) && <section className="panel future"><span>◇</span><h2>{pageTitle}</h2><p>This workspace is reserved for organization-managed records. The evidence and assessment foundation is ready; the next build can add these records without mixing them with automated tenant data.</p><button onClick={() => setPage(page === 'audit' ? 'evidence' : 'overview')}>{page === 'audit' ? 'Open evidence vault' : 'Return to overview'}</button></section>}
